...
- [x] `src/lib/analytics/posthog.ts`: init, isAnalyticsEnabled, capture_exceptions, session_recording
- [x] `src/lib/analytics/events.ts`: trackEvent helper, 9 event constant
- [x] `src/lib/analytics/__tests__/posthog.test.ts`: 2 guard test (TDD)
- [x] `src/components/ErrorBoundary.tsx`: React error boundary + captureException
- [x] `src/components/PostHogProvider.tsx`: SPA pageview tracking via useLocation
- [x] `src/main.tsx`: ErrorBoundary > BrowserRouter > PostHogProvider > App
- [x] Store actions: 7 action'a trackEvent eklendi (addNode, addEdge, importCompose, addRecommendedNode, addServiceFromRegistry, addServiceFromHub, addStack)
- [x] UI components: YamlOutput (copy/download), AISidebar (generate), CommandSearch (5 action)
- [x] `.env.example`, Dockerfile ARGs, PROJECT_SPEC.md tech stack
- **110 unit + 14 MCP unit + 30 E2E test, all passing**
